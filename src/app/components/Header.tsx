      ...
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full">
        <div className="flex justify-between items-center h-full text-sm">
          ...
          <div className="hidden md:flex items-center space-x-6">
            ...
            <button className="px-6 py-3 bg-gradient-to-r from-blue-600 to-sky-600 text-white rounded-lg font-semibold hover:shadow-lg transform hover:scale-105 transition-all">
              Get Started
            </button>
          </div>
          ...
          <button
            onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
            className="md:hidden p-2 rounded-lg hover:bg-gray-100 text-black transition-all"
          >
        ...
        {/* Mobile Menu Panel */}
        <div
          className={`md:hidden absolute left-0 right-0 top-[80px] bg-white/95 backdrop-blur-lg border-t border-gray-200 shadow-lg transform transition-all duration-300 origin-top ${
            mobileMenuOpen
              ? 'opacity-100 scale-y-100 pointer-events-auto'
              : 'opacity-0 scale-y-0 pointer-events-none'
          }`}
        >
          <div className="flex flex-col items-center space-y-2 py-6 px-6">
            {navItems.map(item => (
              <button
                key={item.id}
                onClick={() => scrollToSection(item.id)}
                className={`w-full text-center py-3 rounded-lg text-lg font-medium transition-all ${
                  activeSection === item.id
            ...
            <button className="w-full mt-4 px-6 py-3 bg-gradient-to-r from-blue-600 to-sky-600 text-white rounded-lg font-semibold hover:shadow-lg transform hover:scale-105 transition-all">
              Get Started
            </button>
          </div>
        </div>
