...
//   description:
//     'Manage all delivery orders directly from the app. Track order progress in real time and keep customers informed with instant status updates.',
...
//   description:
//     'Allow your group to easily add items to one shared order using a simple code. Everyone can see and add to the cart from their own mobile phone.',
//   gradient: 'from-orange-500 to-amber-600',
//   image: '/images/groupordering.png',

//   benefits: [
//     'Guests order directly from their phone',
//     'One person manages the whole table order',
//     'No more passing a single menu or tablet'
//   ]
// },
//   {
//     icon: BarChart3,
...
//   title: 'Split Bill',
//   description:
//     'Allow groups to easily divide their bills by items, percentage, or equally. Simplify checkout for dine-in customers and speed up payments.',
//   gradient: 'from-orange-400 to-amber-500',
//   image: '/images/splitbill.jpg',
//   benefits: [
//     'Item-wise or equal split',
...
const features = [
  {
    ...
    benefits: ['Instant menu access', 'Contactless ordering', 'Real-time updates', 'Dietary filters (vegetarian, vegan, gluten-free, allergen warnings)',
      'Item customization (add-ons, modifications, spice levels)',
      'Visual menu with high-quality food images',
      'Popular items and chef recommendations highlighting']
  },
  {
    icon: UtensilsCrossed,
    title: 'Kitchen Display System',
    ...
    gradient: 'from-orange-500 to-amber-500',
    image: '/images/img9.png',
    benefits: ['Real-time order sync', 'Prep time tracking', 'Order prioritization','Audio/visual alerts for new orders',
      'Order timing countdown with overdue warnings',
      'Consolidate multiple orders from same table',
      'Rush order flagging for priority handling',
      'Course sequencing',
      'Estimated ready time sent to waitstaff/customers',
      'Chef notes and preparation reminders per dish']
  },
{
  icon: Bike,
  title: 'Delivery Management',
  ...
  benefits: [
    ...
    'Instant customer notifications',
    'Live delivery status updates',
     'Order acceptance/rejection based on capacity'
  ]
},
  {
    icon: ShoppingBag,
    ...
    gradient: 'from-orange-500 to-amber-500',
    image: '/images/img7.png',
    benefits: ['Ready-time estimates', 'SMS notifications', 'Queue management', 'Takeaway order dashboard',
  ...
  },
  {
    ...
    gradient: 'from-blue-500 to-sky-600',
    image: '/images/img6.png',
    benefits: ['Table status tracking', 'Reservation system', 'Waitlist management',    'Real-time table status',
  ...
  },
  {
    ...
    benefits: ['Instant notifications', 'Request tracking', 'Priority alerts',   'Instant notification to the waiter',
  ...
  image: '/images/groupordering.png',
  benefits: [
     'No more passing a single menu card or tablet device',
      'Table specific session code by host',
      'Hassle free ordering for each guest with session code'
  ...
  image: '/images/splitbill.jpg',
  benefits: [
    'Split bill amount equally among all guests',
      'Unequal splits with custom percentages',
      'Custom tip amount',
    'Multiple payment methods',
    'Fast and accurate billing'
  ]
...
const ImageCard = ({
  ...
  return (
    <div
      className="relative rounded-3xl overflow-hidden bg-linear-to-br from-gray-50 via-white to-gray-100 shadow-[0_8px_30px_-10px_rgba(0,0,0,0.15)] transition-all duration-700 ease-out group"
      style={{
        transform: imageHovered ? 'translateY(-10px) scale(1.02)' : 'translateY(0) scale(1)',
        boxShadow: imageHovered
          ? '0 20px 40px -10px rgba(0,0,0,0.2)'
  ...
  return (
    ...
    >
      ...
      <div className="w-full p-2 lg:w-1/2 space-y-6">
        <div className="flex items-start gap-4">
          <div
            className={`shrink-0 w-14 h-14 sm:w-16 sm:h-16 rounded-2xl bg-linear-to-br ${feature.gradient} flex items-center justify-center shadow-lg transition-transform duration-300`}
            style={{
              transform: imageHovered ? 'scale(1.1) rotate(-5deg)' : 'scale(1)'
            }}
          >
        ...
        {/* Benefits */}
        <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-4 pl-20">
          {feature.benefits.map((b, i) => (
            <li key={i} className="flex items-center gap-2 text-gray-700 font-medium">
              <div
                className={`w-3 h-3 flex items-center justify-center rounded-full bg-linear-to-br ${feature.gradient}`}
              >
...
const FeaturesSection = () => (
  <section className="relative py-20 sm:py-28 lg:py-16 px-4 sm:px-8 lg:px-12 overflow-hidden">
    ...
    <div className="relative max-w-7xl mx-auto">
      {/* Header */}
      <div className="text-center mb-24">
        <div className="inline-flex items-center gap-2 px-6 py-2.5 bg-linear-to-r from-blue-50  to-blue-50 border border-blue-200 rounded-full text-sm font-semibold shadow-sm">
          <span className="bg-linear-to-r text-lg from-blue-700  to-blue-700 bg-clip-text text-transparent">
            Powerful Features
          </span>
        </div>
        ...
        <h2 className="text-5xl lg:text-6xl font-black text-gray-900 mt-6 leading-tight">
          Everything You Need to{' '}
          <span className="block bg-linear-to-r from-blue-600 via-orange-500 to-blue-700 bg-clip-text text-transparent">
            Run Your Restaurant
          </span>
        </h2>
        <p className="text-lg sm:text-xl text-gray-600 max-w-3xl mx-auto mt-5 leading-relaxed">
      ...
      {/* CTA */}
      <div className="text-center mt-28">
        <div className="inline-flex flex-col items-center gap-5 bg-linear-to-br from-blue-50 via-orange-50 to-blue-50 border-2 border-blue-200/50 rounded-3xl p-10 shadow-xl backdrop-blur-sm max-w-2xl mx-auto relative overflow-hidden">
          {/* Decorative gradient overlay */}
          <div className="absolute inset-0 bg-linear-to-br from-blue-100/30 via-transparent to-orange-100/30 pointer-events-none"></div>
         ...
         <button
                ...
                className="relative z-10 group px-8 py-4 bg-linear-to-r from-blue-600 via-orange-500 to-blue-600 bg-size-200 bg-pos-0 hover:bg-pos-100 text-white rounded-full font-semibold shadow-lg hover:shadow-2xl hover:scale-105 transition-all flex items-center gap-2"
              >
                Start Free Trial
           <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
         </button>
